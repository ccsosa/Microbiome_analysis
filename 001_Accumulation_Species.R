...
pacman::p_load(phyloseq, vegan, ggpubr, iNEXT)
################################################################################
################################################################################
accumulation_curve_function <- function(physeq4, option) {
  if (is.null(option) | !option %in% c("iNEXT", "ggrare")) {
...
# #Introducing a method to automatically choose a threshold for rarefaction
# message("Using rarefaction values to try to get a suggested threshold")
#
# #Threshold
# threshold <- 4 # It means difference of two OTUs
# #Filtering to use only rarified data from iNEXT
# message("Using size based approach")
# df_raref <- out$iNextEst$size_based %>%
#   filter(Method == "Rarefaction")  # focus on observed range
...
#   thr <- data.frame(seqs = as.numeric(sample_sums(physeq4)[assemblage[[i]]]),
