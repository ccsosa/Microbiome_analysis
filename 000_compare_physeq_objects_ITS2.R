...
#original_paths
paths <- c(
  "~/00101_20250411JC1K0Y/r_output/ITS2/ecm_physeq.Rdata",
  "~/00101_20250411JC1K0Y/r_output/ITS2_eukaryome/ecm_physeq.Rdata"
  )
#Loading phyloseq objects
physeq_list <- lapply(1:length(markers), function(i) {
  phy_i <- 
...
write.csv(summary,paste0(dir, "/comparisons/ITS2_summary.csv"),row.names = F)
...
x_jacc <- lapply(1:length(levels), function(i) {
  ...
  ggsave(
    filename = paste0(dir, "/comparisons/ITS2_VENN", "_", level, ".pdf"),
  ...
  diag(jacc_mat) <- 1
  write.csv(jacc_mat,paste0(dir, "/comparisons/ITS2_JACCARD", "_", level, ".csv"))
  return(physeq_list2)
})
