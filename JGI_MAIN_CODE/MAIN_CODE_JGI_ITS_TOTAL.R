...
physeq <- readRDS("~/00101_20250411JC1K0Y/r_output/ITS2/physeq_decontam.Rdata")
...
if(!file.exists(paste0(RDS_dir, "/001_filtered_PSD.RDS"))){
  ...
  #getting length
  sp_control <- strsplit(as.character(tax[,7]),"_")
  #getting length of species character 
  sp_control <- lapply(1:length(sp_control),function(i){
    x <- sp_control[[i]]
    x <- length(x)
    # if(length(x)>1){
      # x <- data.frame(Genus = x[[1]],sp = x[[2]])
    # } else{
      # x <- data.frame(Genus = x[[1]],sp = NA)
    # }
    return(x)
  })
  sp_control <- do.call(rbind,sp_control)
  for(i in 1:nrow(tax)){
    if(tax$Genus[[i]] =="?" &
       tax$Species[[i]]=="?" &
       sp_control[[i]] == 1
      ...
      tax$Species[[i]] <- paste0(tax$Genus[[i]],"_sp")
#      tax$Species[[i]] <- "?"
    } else if(
      tax$Genus[[i]] !="?" &
      tax$Species[[i]]!="?" &
  ...
  #replacing taxonomy to use Genus sp. if needed
  ...
  # target_positions <- grep("(Genus$| Family$| Order$| Class$| Phylum$)", UT)
  # UT[target_positions] <- "?"
  # UT <- UT[which(UT!="?")]
  # tax[,7][!tax[,7] %in% UT] <- "?"
  # tax_table(physeq) <- as.matrix(tax)
...
################################################################################
x_kr <- kruskal(Alpha$Alpha$InvSimpson,Alpha$Alpha$Site_name.x,
...
x_ECM$dataset <- "ECM"
x_ECM <- x_AM[complete.cases(x_ECM),]


write.csv(
  x,
  paste0(csv_dir, "/", "001_Correlation_Alpha.csv"),
  na = "",
  row.names = F,
  quote = T
)
...
write.csv(
  x_ECM,
  paste0(csv_dir, "/", "001_Correlation_Alpha_ECM.csv"),
  na = "",
  row.names = F,
  quote = T
)
...
physeq4_fix <- tax_fix(physeq4,unknowns = c("?"))
bp1 <- compo_plot(physeq=physeq4_fix,
                       level="Order",
                       ntop=10,
                       column="Habit")
  
bp2 <- compo_plot(physeq=physeq4_fix,
                  level="Family",
                  ntop=10,
                  column="Habit")

bp3 <- compo_plot(physeq=physeq4_fix,
                  level="Genus",
                  ntop=10,
                  column="Habit")
bp4 <- compo_plot(physeq=physeq4_fix,
                  level="Species",
                  ntop=10,
                  column="Habit")
...
################################################################################
#Sites
bp1_S <- compo_plot(physeq=physeq4_fix,
                  level="Order",
                  ntop=10,
                  column="Site_name.x")

bp2_S <- compo_plot(physeq=physeq4_fix,
                  level="Family",
                  ntop=10,
                  column="Site_name.x")

bp3_S <- compo_plot(physeq=physeq4_fix,
                  level="Genus",
                  ntop=10,
                  column="Site_name.x")
bp4_S <- compo_plot(physeq=physeq4_fix,
                  level="Species",
                  ntop=10,
                  column="Site_name.x")
...
dev.off()
################################################################################
cor_table <- cor_var_function(ps = physeq4_fix,padj_threshold=0.05)
write.csv(
  cor_table,
  paste0(csv_dir, "/", "006_Correlation_Table_general.csv"),
  na = "",
  row.names = F,
  quote = T
)

physeq_clr2_sp_sp <- microViz::tax_agg(physeq4_fix,"Species")
physeq_clr2_sp_sp <- transform(physeq_clr2_sp_sp,"compositional")
x <- data.frame(otu_table(physeq_clr2_sp_sp))
x <- x[which(row.names(x)=="Oidiodendron_chlamydosporicum"),]
tol <- 1e-12
x[abs(x) < tol] <- NA
...
y <- data.frame(SampleID = row.names(y),
                var =as.numeric(y$Bulk.Density..g.cm..),
                Site=y$Site_name.x,
                Habit = y$Habit
)
...
ggscatter(xy, y = "freq", x = "var", color = "Habit", #"Habit",#"Site_name.x",
          xlab = "Magnesium Saturation [%]",
          
          ylab = "Fusicolla (relative abundance)")+
...
# xy <- xy[complete.cases(xy),]
# xy <- xy[xy$var %in% na.omit(remove_outliers(xy$var)),]
# xy <- xy[which(xy$freq<0.0019),]
# cor.test(xy$freq,xy$var,method="spearman")
# plot(xy$freq,xy$var)


################################################################################
AM_ps <- subset_samples(physeq4_fix,Habit=="AM")
...
####################################################################
ECM_ps <- subset_samples(physeq4_fix,Habit=="ECM")
...
name <- "Sites"
Upsetplot_compo(physeq4_fix,graph_dir = graph_dir,column=column,levels=levels,name=name)
