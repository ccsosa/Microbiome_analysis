...
physeq <- readRDS("~/00101_20250411JC1K0Y/r_output/SSU_eukaryome/amf_physeq.Rdata")
...
if(!file.exists(paste0(RDS_dir, "/001_filtered_PSD.RDS"))){
  ...
  for(i in 1:nrow(tax)){
    if(tax$Genus[[i]] =="?" &
      ...
      tax$Species[[i]] <- paste0(tax$Genus[[i]]," sp.")
#      tax$Species[[i]] <- "?"
    } else if(
      tax$Genus[[i]] !="?" &
      tax$Species[[i]]!="?"
...
if(!file.exists(paste0(RDS_dir, "/002_rarefied_PSD_4.RDS"))){
 ...
 ps_rare3 <- accumulation_curve_threshold_function(physeq4,
                                                   threshold_label = "10000",
                                                   option = "SRS",Cmin = 10000)
...
ggscatter(data, y = "Clay [%]", x = "pielou", color = "Habit", #"Habit",#"Site_name.x",
          ylab = "Clay (%)",
