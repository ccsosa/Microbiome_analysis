...
################################################################################
################################################################################
accumulation_curve_threshold_function <- function(physeq4, option,threshold_label,Cmin) {
  if (is.null(option) | !option %in% c("SRS")) {
  ...
  if(option=="SRS"){
    ...
    ggrare_plot <- ggrare_plot +
      labs(title = "",
           ...
           y = "Richness")
    ggsave(
      paste0(graph_dir, "/", "001_rarefaction_curve_ggrare_",threshold_label,".pdf"),
      ggrare_plot,
      scale = 0.9,
      width = 25,
      height = 14,
    ...
    pdf(paste0(graph_dir, "/", "001_rarefaction_curve_SRS_",threshold_label,".pdf"))
    ...
    saveRDS(ps_rare, paste0(RDS_dir, "/002_rarefied_PSD_4",threshold_label,".RDS"))
    message("returning rarefied file to calculate Alpha diversity")
    message(paste0("Sample with: minimum read counts: ",min(sample_sums(phy5)),
                   " /Threshold used: ",Cmin))
    write.csv(data.frame(method="SRS",threshold=threshold_label),
              paste0(csv_dir,"/001_threshold_4_",threshold_label,".csv"),row.names = F)
